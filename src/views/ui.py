...
def create_main_window():
    ...
    btn_view_clients = ttk.Button(
        ...
        command=lambda: open_view_clients_window(root)
    )
...
def open_view_clients_window(root):
    ...
    btn_edit_client = ttk.Button(
        root, text="Modifier Client",
        command=lambda: modify_client(tree)
    )
    btn_edit_client.grid(column=2, row=0, padx=5, pady=5)

    btn_delete_client = ttk.Button(
        root, text="Supprimer Client",
        command=lambda: delete_client(tree)
    )
...
def modify_client(tree):
    ...
    if client:
        ...
        def save_changes():
            ...
            view_clients()  # Refresh the client list

        save_button = tk.Button(modify_window, text="Enregistrer les modifications", command=save_changes)
